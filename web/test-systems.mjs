...
assert.match(powerSource, /2026 · MGU-H REMOVIDO/, 'Power deve exibir o limite temporal de 2026');
...
assert.ok(fuelCellBounds[2][1] < POWER_FUEL_LAYOUT.cockpitRegionZ[0], 'a célula deve ficar atrás da região do cockpit');
assert.ok(survivalCellBounds[2][0] > fuelContainmentBounds[2][1], 'o tanque deve ficar longitudinalmente depois da survival cell');
assert.ok(fuelContainmentBounds[2][0] > powerBlockBounds[2][1], 'o tanque deve ficar longitudinalmente antes do motor');
