...
export function createWorkbench({scene,model,driver,engine,garage,systems,controls,camera,moveCamera,closeEngine,showCar,bench=null,onTab=()=>{}}){
 ...
 for(const panel of panels){const title=panel.querySelector('h3')?.textContent||'';panel.dataset.labPanel=title.startsWith('MOTOR')?'engine':panel.id==='air-panel'?'air':title.startsWith('CAPACETE')?'helmet':title.startsWith('PILOTO')?'driver':title.startsWith('AMBIENTES')?'world':title.startsWith('SISTEMAS')?'systems':title.startsWith('SOM')?'sound':'car';}
 ...
 const benchOn=()=>Boolean(bench)&&bench.wanted;
 function frameBench(){
  // Só o V12 tem modelo próprio. Sem ele (V6), a aba volta a mostrar o carro no box.
  if(!benchOn()){showCar();moveCamera(new THREE.Vector3(3.2,1.25,-4.6),new THREE.Vector3(0,.55,-1.2));return;}
  // A bancada fica na origem: o plano de corte vive no espaço do mundo e não pode ser deslocado.
 ...
 function refreshPanels(){for(const p of panels)if(p.dataset.labPanel!=='air')p.hidden=p.dataset.labPanel!==active;document.querySelector('.panel-heading h2').textContent={car:'Carro',engine:'Motor V6',helmet:'Capacete',driver:'Piloto',world:'Ambientes',systems:'Sistemas reais',sound:'Som'}[active];document.querySelector('.panel-heading .badge').style.display=active==='car'?'':'none';}
 function setTab(next){
  const previous=active;if(active==='systems')systems?.deactivate();
  ...
  if(active==='helmet'||active==='driver'){rebuild();}else if(active==='engine')$('engine-open').click();else if(active==='systems')systems?.show('overview');else if(active!=='sound')showCar();
  onTab(active,previous);
  if(active==='sound')frameBench();
 }
 ...
 refreshPanels();
 return {get inspecting(){return active==='helmet'||active==='driver'||(active==='sound'&&benchOn());},view,setTab,refreshSound(){if(active==='sound')frameBench();},
  update(){const isolated=(active==='helmet'||active==='driver')&&mode==='isolated',bancada=active==='sound'&&benchOn();inspection.visible=isolated;model.visible=bancada?false:!isolated;if(isolated||bancada)engine.root.visible=false;},
