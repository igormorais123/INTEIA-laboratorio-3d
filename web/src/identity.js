...
export function brandSVG({light=false,compact=false}={}){
 ...
 return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 116" role="img" aria-label="INTEIA — Laboratório 3D"><g transform="translate(66 86) skewX(-12)"><text text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-weight="700" font-size="66" fill="${red}">Box</text></g><path d="M141 19V96" stroke="${ink}" stroke-opacity=".22"/><g transform="translate(183 17) skewX(-12)">${glyphs.map(([letter,d,accent])=>`<path data-letter="${letter}" fill="${accent?red:ink}" fill-rule="evenodd" d="${d}"/>`).join('')}</g><text x="174" y="106" font-family="Arial, sans-serif" font-size="14" font-weight="500" letter-spacing="4.5" fill="${ink}">LABORATÓRIO 3D</text></svg>`;
}
export function drawBrand(ctx,width,height,{light=false}={}){
 ...
 const ink=light?'#f0f2f3':'#202930',red='#d92135';
 ctx.save();ctx.translate(66,86);ctx.transform(1,0,-Math.tan(12*Math.PI/180),1,0,0);ctx.fillStyle=red;ctx.font='700 66px Arial';ctx.textAlign='center';ctx.fillText('Box',0,0);ctx.restore();
