...
export function createSennaDriver(model, mechanics) {
 ...
 const helmet=createHelmet1991({detail:96});root.add(helmet.root);
 const fit={height:0,foreAft:0,pitch:0};
 function applyFit(){helmet.root.position.set(0,.737+fit.height,.075+fit.foreAft);helmet.root.rotation.set(.035+fit.pitch,0,0);}
 applyFit();
