...
export async function applyInteiaBranding(model, mechanics) {
  ...
  const [map,crestMap,sponsorMap]=await Promise.all(['inteia-nome-oficial.svg','inteia-escudo-oficial.svg','inteligencia-mil-grau-transparent.png'].map(name=>loader.loadAsync(`./assets/${name}`)));
