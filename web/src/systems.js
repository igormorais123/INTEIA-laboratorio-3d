...
export const SYSTEM_CATALOG=Object.freeze([
 ...
 {id:'safety',spread:[0,1.5,.4],number:'11',label:'Segurança',short:'SAFETY',chapter:'17:03',color:'#ff4d67',description:'Halo e encosto de cabeça com a mesma malha da carroceria do carro v2 (coincidem com o que se vê por fora), estrutura principal de capotamento no airbox, cone de impacto dentro do envelope do nariz, estrutura de impacto traseira com a luz de chuva, tubos anti-intrusão laterais, três cabos de retenção por roda, extintor e chaves externas. Camada de proteção didática, não homologada.'},
 {id:'cockpit',spread:[0,1.0,1.2],number:'12',label:'Cockpit e retenção',short:'COCKPIT',chapter:'17:51',color:'#d9a8ff',hero:[.9,.9,-1],description:'Assento moldado dentro da cavidade real do cockpit do carro v2, arnês de seis pontos com fecho rotativo, HANS com tirantes, acelerador no pé direito (o freio fica no esquerdo), acolchoados de perna, hidratação, painel de chaves e rádio; leitura didática do espaço do piloto, não ergonomia validada.'},
 {id:'wheel',spread:[0,1.3,1.5],number:'13',label:'Volante e comandos',short:'VOLANTE',chapter:'19:17',color:'#8db5ff',hero:[-.3,.3,-1],description:'Leiaute inspirado no protótipo Ferrari 2026: corpo de carbono achatado com empunhaduras verticais, display largo com quinze LEDs, botões coloridos com legendas (N, P, OT de override manual, AA de aerodinâmica ativa), balancins de freio, três seletores grandes, rolos de polegar, borboletas de marcha e de embreagem, engate rápido e cabo helicoidal. Clique em cada comando para ler a função; não simula uma ECU real.'},
 {id:'sensors',spread:[0,-1.0,-.7],number:'14',label:'Sensores',short:'SENSORES',chapter:'22:11',color:'#7ee5f2',description:'Tubo de Pitot, sensores de velocidade de roda com anéis dentados, câmeras infravermelhas de pneu, lasers de altura, ECU padrão, gravador de acidentes, IMU, células de carga, termopares, transceptor e antena com chicote e fluxos de dados carro → box; esquema didático, não telemetria real.'}
]);
...
export function createSystems({scene,model,mechanics,engine,driver,garage,camera,canvas,moveCamera,showCar=()=>{},onClose=showCar,mobile=false,assetUrl=SYSTEMS_ASSET}){
 ...
 updateUI('overview');
 let time=0;
 return {root,groups,show,view,activate:show,deactivate:()=>setEnabled(false),setPaused(value){paused=Boolean(value);},setERSContext,load,reset:()=>{explodeAmount=0;revealSpread=0;applyExplode();show('overview',false);},setSpread(value){revealSpread=Math.min(Math.max(Number(value)||0,0),1);if(!enabled)applyExplode();},get spread(){return revealSpread;},
