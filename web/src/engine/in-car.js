...
export function createInCarEngine({scene,renderer,camera,model,mechanics,target,mobile=false,signal=null,offstage=()=>[]}){
 ...
 return {
  ...
  update(dt,shot,paused){
   ...
   const exploded=mechanics.amount>.01;
   const open=(shot?shot.open:0)*reveal,on=ready&&open>.001&&!exploded;
   if(on!==split)setSplit(on);
   root.visible=ready&&(on||exploded)&&!mechanics.isolated;
   if(!root.visible)return;
   if(on){poseCover(open);for(const [base,clone] of [...coverMaterials,...restMaterials]){clone.color.copy(base.color);for(const key of ['roughness','metalness','clearcoat','clearcoatRoughness','wireframe'])clone[key]=base[key];}}
   light.intensity=on?shot.light*reveal*WORK_LIGHT:0;
   const cut=on?shot.cut:0;
