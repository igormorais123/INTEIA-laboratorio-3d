...
[test-mechanics.mjs](../../web/test-mechanics.mjs) carrega a base GLB, testa pivôs e 20 ciclos, restauração, isolamento e arraste; grava a evidência mecânica na raiz. [test-aerodynamics.mjs](../../web/test-aerodynamics.mjs) verifica unidades, escalas V²/V³, vento relativo, densidade, limites e ausência de coeficientes. Não há integração contínua remota: build e testes rodam localmente antes de cada publicação no ChatGPT Sites (ver [PUBLICACAO.md](../PUBLICACAO.md)). Relatórios antigos do Blender e screenshots são evidência da execução registrada, não certificação de um export futuro.
