...
Antes de atualizar, confira `git status --short`. Mudanças paralelas devem ser preservadas e isoladas antes de gerar um snapshot para commit. Durante esta entrega houve outra tarefa em `docs/mapeamento-detalhado/` e uma revisão simultânea de materiais, luzes e ambientes. A documentação foi iniciada em uma worktree de `1ae4fd1`, na branch `codex/mapas-inteia`, e atualizada sobre `0346e58` após a conclusão dos materiais/luzes. Ao integrar outras linhas de trabalho, regenere os mapas sobre o código integrado e reveja as descrições humanas.
