...
Abra o endereço exibido no terminal. Para validar e gerar uma cópia estática: `npm test` e `npm run build`. O laboratório gera `web/index.html`; preserve também os arquivos de `web/assets/` e `web/THREE-LICENSE.txt`. Não abra o HTML por duplo clique: módulos e carregamento de assets precisam de um servidor HTTP. Não são necessárias chaves de API.
